...
- **客户价值分群**：结合产品分类特征基于MiniBatchK-Means划分7类客户群体，
  通过Matplotlib/Seaborn可视化，定位超级VIP客户（0.77%群体贡献32%营收），设计定制化策略（VIP会员制、捆绑销售等）
- **集成学习优化**：对比6种模型（随机森林、梯度提升等），通过投票分类器（权重调优）实现客户类别预测准确率84.46%，
  Matplotlib可视化混淆矩阵与特征重要性，提升业务可解释性
### 2. 关键成果
...
### 客户聚类目标
-识别不同类型的客户群体
-通过聚类分析，将客户划分为具有相似消费行为的多个群体，识别高价值客户、潜在流失客户等典型画像。

-了解各客户群体的行为特征
-分析各类客户在订单频率、消费金额、品类偏好等维度上的差异，辅助市场营销策略制定。

-通过聚类方法对客户进行分层管理
-实现客户的精细化运营，为客户推荐更合适的产品和服务，并提升客户生命周期价值。
### 客户聚类采用的方法
